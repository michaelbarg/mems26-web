...
## לוג-המשימות — `docs/plans/TASK_LOG.md` הוא מקור-האמת היחיד (מייקל 2026-08-17)

**כל סוכן, בתחילת כל סשן: לקרוא `docs/plans/TASK_LOG.md`.** שם מה פתוח, מה
הסטטוס, ומה **הצעד הבא** לכל פריט — כדי שאפשר להרים משימה קר, בלי להיזכר.

**סוכן-Cowork: לקרוא גם `docs/runbooks/COWORK_DAILY_READ.md`** — סדר-הקריאה,
הפקודות שמוציאות את אמת-היום, ו-10 המלכודות שכל אחת מהן כבר הפכה דוח לשקר
(לוג-האפליקציה הוא `backend.err.log` ולא `backend.log` · פיד-ההחלטות חסום ל-200 ·
`CURRENT_DATE` שקרי אחרי חצות · אפס-עסקאות ≠ מערכת-מתה · גודל-עסקה נקרא
מ-`ruled_contracts()` ולא מ-`.env`). זה **לא** מרשם-משימות מתחרה — זו שיטת-הבדיקה.

- **פתחת משימה?** שורה חדשה: `| T-xx | תיאור | סטטוס | מי | הצעד הבא |`.
- **התקדמת?** לעדכן את "הצעד הבא". לא למחוק היסטוריה.
- **סגרת?** ל-✅ **וגם** שורה מאומתת ב-`STATUS_BOARD.md` (ממצא → תיקון → ראיה).
  `task_log_guard` נכשל על ✅ בלי שורת-לוח.
- **הודעה לסוכן אחר?** ל-`LIVE_CHANNEL.md`, לא ללוג.
- **אל תיצור קובץ-משימות מתחרה.** `OPEN_TASKS*.md` ודומיו נכשלים בבודק — שני
  מרשמים פירושם שאף אחד מהם אינו המקור. זה בדיוק מה שהרג את
  `SOURCE_OF_TRUTH.md` (התיישן 54 יום) ואת `SYSTEM_MANIFEST.md` (32 יום).

נאכף ע"י `python3 scripts/task_log_guard.py` (רץ בתוך `fire_drill.py`, כלומר
לפני כל סשן): נכשל על לוג שלא עודכן 3 ימים, פריט בלי סטטוס או בלי צעד-הבא,
✅ בלי אימות, או קובץ-משימות מתחרה.

## cc-macbook עובד בלי שאלות (מייקל 2026-09-14 09:30: "שאלות תוקעות את העבודה")

**סוכן-cc לא שואל — הוא מחליט, מתעד וממשיך.** מייקל אישר מראש את כל הפריטים שבהזמנות
החתומות `cowork-dev` ב-`LIVE_CHANNEL.md` (סדרת "פריט N/8" ב-`docs/handoff/cc_orders/`).
- "האם להמשיך / לקמט / להריץ הרנס / לערוך קובץ X?" — **התשובה תמיד כן.** אין להדפיס שאלה
  ולחכות; שאלה מודפסת = הפרה של ההזמנה.
- דו-משמעות בפריט ⇒ בוחרים את הפרשנות שההזמנה מציינת כ-golden, כותבים אותה בשורת-ה-LOG
  ("החלטתי X כי …") וממשיכים. אם אי-אפשר לסיים ⇒ **NOT-DONE מפורש** עם השורה הגולמית
  והפריט הבא — לא עצירה.
- **הגבול היחיד שעוצר** (ואז כותבים את השאלה בסוף הדיווח, לא באמצע העבודה): שינוי
  שמהווה סיכון-מסחר בלי פסיקה כתובה — `.env`/דגל-env חדש/דגלי-גודל/`RISK_*`/ריסטארט/
  פוזיציות/פקודות/op=EXIT/FLATTEN. כל השאר — בסמכות.
