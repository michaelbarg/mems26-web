...
**שער S (ראשון בוקר, סים)** ─► S0 Sim ON → S1–S5 + Si1–Si3 → ⛔ S-LIVE חזרה-ללייב
C + S-LIVE ✅ ───────────────► D (בוקר שני) → GO/NO-GO 16:25
```
...
## שער S — סשן-ראשון (19/07, מחוץ לשעות מסחר) · מקור: `SUNDAY_SIM_SESSION_2026-07-19.md`

**מטרה:** להביא ראיות שכל תבנית מאושרת יורה בסוג-היום שלה — ואז **להחזיר ללייב**.
**חוק-בטיחות:** לפני כל פקודת-מסחר — `is_sim=1` מ-`sierra_state.json`. `is_sim=0` → עצור מיד.
⛔ **הצ'קליסט / שער S לא נסגרים בלי S-LIVE** — התקלה הקלאסית: לסיים סשן-סים ולפתוח יום שני על סים.

### שערי-אנוש (מייקל) — חובה

| מזהה | משימה | בעלים | תלות | קריטריון-סיום | סטטוס |
|---|---|---|---|---|---|
| **S0** | ⏸ **Sim Mode ON** — משחרר S1 + כל E2E | **מייקל** | לפני כל פקודת-סים | סיירה: Trade Simulation Mode **ON**. פלט: `sierra_state.json` → `is_sim=1` (+ flat, מחיר-שפוי). שורה ב-LIVE_CHANNEL `S0 SIM ON`. **בלי זה — אפס PLACE/PLACE_STOP** | ⬜ ⏸ אנושי — חוסם S1/E2E |
| **S-LIVE** | ⏸ **חזרה ללייב** — פריט-סגירה חובה | **מייקל** → **cowork מאמת** | אחרי S1–S5 + Si* (או אחרי עצירה מסודרת) | סיירה: Simulation Mode **OFF**. פלט: `is_sim=0` + flat + מחיר-שפוי. שורה `S-LIVE DONE` + פלט גולמי. ⛔ **בלי S-LIVE — שער S = לא-סגור · יום שני = NO-GO** | ⬜ ⏸ אנושי — אחרון |

### cc-macbook — S1–S5

| מזהה | משימה | בעלים | תלות | קריטריון-סיום | סטטוס |
|---|---|---|---|---|---|
| **S1** | אימות-סים PLACE_STOP (השלמת משימה 1ב / =A1.6) | **cc-macbook** | **S0 ✅** · A1.5 ✅ | יתום ‎-2 בסים → ORPHAN=1 **בסביבת-סים בלבד** → סטופ בצד/מחיר · `working` 0→1 · פוזיציה לא גדלה · מראה LONG · חד-פעמיות. אם `r=-1` על יתום-נקי: עצור+דווח. פלט ב-LIVE_CHANNEL | ⬜ חסום-S0 |
| **S2** | מטריצת-הדמיה מלאה (הלב) — לוגיקה + miss-audit + E2E סים | **cc-macbook** → cowork | **S0 ✅** לחלק ה-E2E | (1) `sim_matrix.py` ירוק. (2) `audit_pattern_miss --date` 15/16/17. (3) E2E: לכל סוג-יום×תבנית לא-SKIP — כניסה→סולם→MODIFY_STOP→יעד→BE→S6→סגירה. **תוצר: טבלת 7×תבניות ✅/❌ + ראיה.** ❌ → תיקון+ריצה | ⬜ (E2E חסום-S0) |
| **S3** | 2 כשלי-סימולציה: Neutral_Center×HTLB · Neutral_Extreme×TLB | **cc-macbook** | חופף A3 | `sim_matrix` / E2E — שני התאים עוברים או מתועדים SKIP מכוון. פלט | ✅ cc 07-19 (112/0; cowork לאמת) |
| **S4** | הרחבת `audit_pattern_miss` ל-TLB/HTLB/VEGAS/GHOST/FAMIR/DBDT | **cc-macbook** | חופף A4 | `--selftest` + ריצת 07-17 מפיקה קריטריונים ל-6; פלט | ✅ cc 07-19 (11 תבניות; cowork לאמת) |
| **S5** | 6 כשלי סיווג-יום — מיושנים מול דוקטרינה או נסיגת-מסווג? | **cc-macbook** | — | קביעה מתועדת + טסטים ירוקים או פסיקה. פלט pytest | ✅ cc 07-19 (40 passed — מיושנים; cowork לאמת) |

### cc-imac — 3 משימות (סים בלבד · לא לחמש)

| מזהה | משימה | בעלים | תלות | קריטריון-סיום | סטטוס |
|---|---|---|---|---|---|
| **Si1** | שכפול-אימות מטריצת S2 (הצלבה מול MacBook) | **cc-imac** | S2 התחיל/יש פלט-Mac; iMac `is_sim=1` | אותן פקודות מטריצה על iMac. אם תוצאה ≠ MacBook → ממצא-סביבה ב-LIVE_CHANNEL. פלט גולמי | ⬜ |
| **Si2** | S6-EOD 07-17 (פריט S-11) | **cc-imac** | — | `python3 scripts/s6_eod_report.py --date 2026-07-17` + ממצאי-CRITICAL ל-`task_board.json`. פלט | ⬜ |
| **Si3** | ריפליי-סיווג 15/16/17-07 (תמיכה ב-S5) | **cc-imac** | S5 / דוקטרינת מייקל | ריפליי מראה האם המסווג מייצר את קריאות מייקל (16/07: Normal→Neutral_Center→Neutral_Extreme בזמנים). פלט | ⬜ |

**מעקב מהיר S:** `⏸S0⬜ → S1⬜ → S2⬜ → S3✅ → S4✅ → S5✅ · Si1⬜ Si2⬜ Si3⬜ → ⛔⏸S-LIVE⬜`
**סגירת שער S = רק כש-S-LIVE ✅** (גם אם S1–S5/Si* ירוקים).
...
| מזהה | משימה | בעלים | תלות | קריטריון-סיום | סטטוס |
|---|---|---|---|---|---|
| **D1** | `is_sim=0` · מחיר-שפוי · **flat** | **cowork-dev** מאמת | **S-LIVE ✅** + C | קריאת `sierra_state.json`: `is_sim=0`, `position_qty=0`, `working_orders=0`, מחיר ≠ 996150 ובטווח-שוק סביר. פלט גולמי | 🔴 |
...
8. ⛔ **אחרי סשן-סים — חובה S-LIVE** (`is_sim=0` אומת). בלי זה הצ'קליסט לא נסגר ויום שני = NO-GO.
...
| 5 | **cc-imac** בשער S: Si1–Si3. חוק סוחר-יחיד — לא לחמש | MacBook+iMac לא חמושים יחד |
| 6 | אין כפילות-בעלים בתוך הצ'קליסט; כל שורה בעלים יחיד (A6/C2 משותפים מפורשים) | — |
| 8 | **כפילות מכוונת:** S1≈A1.6 · S3≈A3 · S4≈A4 · S2⊃C1/C2 — שער S הוא סדר-הביצוע בראשון; A/C נשארים למעקב | לסמן ✅ בשני המקומות אחרי cowork |
| 9 | ⛔ **S-LIVE** הוא פריט-הסגירה — בלי `is_sim=0` אומת, שער S פתוח גם אם כל השאר ירוק | לא לסגור צ'קליסט / לא GO ביום ב' |
...
| **S** | מייקל (S0, S-LIVE) · cc-macbook (S1–S5) · cc-imac (Si1–Si3) | 2 אנוש + 5 + 3 |
| **D** | cowork-dev (D1,D3,D4,D5) · מייקל (D2) | 5 |
| **סה״כ שורות-עבודה** | | **22 + P + שער S** |

**סטטוס S עכשיו:** S0/S1/S2/Si* פתוחים · S3–S5 ✅ (cc, ממתין-אימות-cowork) · **S-LIVE חובה בסוף**.
**סדר ראשון:** S0 → S1 → S2 (+ Si1 במקביל) → Si2/Si3 → S-LIVE → אחר-כך D ביום ב'.
