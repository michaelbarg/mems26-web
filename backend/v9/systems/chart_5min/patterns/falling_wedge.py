"""C4a. Falling Wedge — both lines falling, converging. Bullish."""
...
def detect_falling_wedge(bars: List[Bar]) -> PatternResult:
    """Detect Falling Wedge in 15-30 bars.

    Both trendlines slope downward but converge (support line is steeper
    than resistance line). Bullish reversal pattern.
    """
    if len(bars) < 12:
    ...
    # Lines must converge: support falls faster than resistance
    # (l_slope more negative than h_slope)
    if l_slope >= h_slope:
        return PatternResult()

    last = window[-1]
    pattern_height = max(h_prices) - min(l_prices)
    breakout = last.c > h_prices[-1]
