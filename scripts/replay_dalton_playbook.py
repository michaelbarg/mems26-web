...
def main():
    ...
    for day in sessions:
        ...
        for t in trades:
            ...
            # Direction hint — layered (same logic as gateway):
            # Layer 1: dir_bias from classify or opening direction
            # Layer 2 (C only): IB extension overrides when ext > 0
            dir_hint = None
            phase = "C" if il_hhmm >= "17:30" else ("B" if il_hhmm >= "16:45" else "A")
            # Layer 1: dir_bias or opening direction (ALL phases)
            ...
            # Layer 2 (C): IB extension overrides when ext > 0
            if phase == "C" and bar_idx >= 12:
