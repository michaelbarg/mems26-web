...
def main() -> int:
    ...
    # ---- emit ----
    # NOTE: intentionally NO generation timestamp or `.env` mtime is written into the
    # index. Those change on every run (and on any git checkout / snapshot restore that
    # bumps file mtimes) even when the flags themselves are unchanged, which made
    # `mems26_verify.sh` report false "index drift". The output is now byte-deterministic
    # for unchanged inputs; git already records WHEN the file last changed.
    out = []
    ...
    out.append("")
    out.append("_Generated by `scripts/gen_flag_index.py` (deterministic — no timestamp) · "
