...
export function ChartV5a() {
  ...
  return (
    ...
    <svg ref={svgRef} data-testid="chart-svg" viewBox={`0 0 ${W} ${H}`}
      ...
      {/* W5.10: X-axis labels — evenly spaced by bar index (TV-parity) */}
      {(() => {
        if (allBars.length < 2) return null;
        const labelCount = 8; // ~8 labels across chart width
        const stride = Math.max(1, Math.floor(allBars.length / labelCount));
