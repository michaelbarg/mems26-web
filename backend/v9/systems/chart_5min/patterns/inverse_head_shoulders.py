"""C1b. Inverse Head & Shoulders — bullish reversal."""

from __future__ import annotations
from typing import List
from backend.v9.systems.chart_5min.models import Bar, PatternResult
from ._helpers import find_pivots, swing_highs, swing_lows, prices_near


def detect_inverse_head_shoulders(bars: List[Bar]) -> PatternResult:
    """Detect Inverse H&S (bullish) in 15-25 bars."""
    if len(bars) < 12:
        return PatternResult()

    window = bars[-min(len(bars), 30):]
    pivots = find_pivots(window, lookback=2)
    highs = swing_highs(pivots)
    lows = swing_lows(pivots)

    if len(lows) < 3 or len(highs) < 2:
        return PatternResult()

    for i in range(len(lows) - 2):
        ls, head, rs = lows[i], lows[i + 1], lows[i + 2]

        # Head must be lowest
        if head.price >= ls.price or head.price >= rs.price:
            continue

        # Shoulders near equal
        if not prices_near(ls.price, rs.price, 0.20):
            continue

        nl_highs = [h for h in highs if ls.index < h.index < rs.index]
        if len(nl_highs) < 1:
            continue

        neckline = max(h.price for h in nl_highs)
        bar_count = rs.index - ls.index + 1

        last = window[-1]
        if neckline <= head.price:
            continue

        completion = min(1.0, (last.c - neckline) / (neckline - head.price) + 0.5) \
            if last.c > neckline else (neckline - last.l) / (neckline - head.price) * 0.5

        if completion < 0.4:
            continue

        pattern_height = neckline - head.price
        entry = neckline
        stop_price = rs.price
        t1 = neckline + pattern_height * 0.618
        t2 = neckline + pattern_height
        t3 = neckline + pattern_height * 1.618

        return PatternResult(
            detected=True,
            pattern_id="inverse_head_shoulders",
            group="C",
            direction="LONG",
            completion=min(completion, 1.0),
            bar_count=bar_count,
            method="Geometric",
            potential_r="5-10R",
            key_levels={
                "left_shoulder": ls.price,
                "head": head.price,
                "right_shoulder": rs.price,
                "neckline": neckline,
                "target": neckline + pattern_height,
                "stop": stop_price,
            },
            confidence=0.6 + 0.2 * min(completion, 1.0),
            entry_price=entry,
            stop=stop_price,
            targets=[t1, t2, t3],
        )

    return PatternResult()
