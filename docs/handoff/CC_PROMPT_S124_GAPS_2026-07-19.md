# CC Prompt — S124 GAPS (G1–G8)

**סטטוס 2026-07-19 13:30:** תור + ביקורת + לוח מוכנים ב-git.  
**GO לקוד:** רק אחרי שורת-LOG ממייקל `G#: לתקן` (או `לשנות-כך`). בלי זה — קריאה בלבד / המתנה.
...
**פנקס:** `docs/handoff/GAP_REGISTER.md` (מיפוי S124↔G-01…)

## חוק הפעלה
1. `git pull`. קרא את שורת-הלוח לפער הפעיל + LOG האחרון.
...
## סדר צפוי (אחרי שמייקל מאשר G0)
G1 (paint) → G2 (detection) → G3 (Flag T2; ניתן למזג עם G2 אם אושר) → G4 (prelock=הדלקה) → G5 (UI) → G6 (fallback) → G7 (FIXED_4 — פסיקה נפרדת) → G8 (דוקטרינה).
