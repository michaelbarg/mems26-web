...
**⏰ עדכון-בוקר 07-24 10:35 (cowork):** לא רצת בלילה — עכשיו זה ריצת-בוקר. פתיחה 16:30 IL.
**דדליין 15:00 IL. סדר-עדיפויות: Phase 0 → 1 → 2 (חובה-להיום) → 5.1+5.2 (endpoint+chip) → 3 → 5.3.**
Phase 6 בוצע-והודלק ע"י cowork (דלג). השורט-העירום נסגר (pos=0) — אין מגבלת-פוזיציה, אבל עדיין הכל flag-OFF;
cowork מאמת ומדליק לפני הפתיחה. עדכן שורת-סטטוס ב-LIVE_CHANNEL אחרי כל Phase שנסגר.
...
2. **פרונט (מפרט-מייקל 07-24 11:00 — מחייב):** רובריקת **"פתיחה"** בפאנל-הצד, **מבנה זהה לרובריקת מערכת-2** (אותה קומפוננטת-סקשן), ממוקמת **מעליה**, **בצבע לבן** (כותרת+מסגרת white — לא צבע-מערכת). תוכן: סוג-הפתיחה (opening_type) + כיוון + confidence + טריגר-הירי כשקיים (DRIVE/TEST/ORR/PULLBACK). "PENDING" לפני 16:35 — סוג+כיוון+confidence, צבע לפי כיוון, "PENDING" לפני 16:35. ניזון מ-`/api/v9/open_type/current`, polling **15000ms** (רצפות-P30 — אסור מהיר-יותר). קומפוננטה קטנה, אל תיגע בקיים.
