...
_Generated 2026-06-30 15:27 · `.env` last modified 2026-06-29 16:32 · scan dirs: backend, bridge_

**Legend:** ✅ ON · 🔴 OFF · 🟡 ON·inert (set ON but superseded at runtime) · 🔢 numeric param · ⚪ not built.

**Summary:** 68 documented · 43 ON (of which 3 inert) · 11 OFF (3 standing-OFF) · 13 numeric params · 1 awaiting backtest · 1 rejected · 1 not built.
...
| S2_CVD_DETECTION_V1 | 🔴 OFF | unset → "" (empty → OFF) | "" (empty → OFF) | CVD confirmation inside S2 detection geometry: REACTIVE requires absorption (entry-bar delta OR price/CVD divergence), INITIATIVE requires net delta with-flow. Fail-open on stale CVD. | OFF (default, SHADOW only). 2026-06-29: INITIATIVE_SHORT fired without CVD confirmation despite +3,403 absorption. Michael sign-off required. | `backend/v9/systems/five_min/five_min_system.py:716`<br>`backend/v9/systems/five_min/five_min_system.py:759`<br>`backend/v9/systems/five_min/five_min_system.py:866`<br>(+1) | deferred; docs/handoff/CC_STAGE2_CVD_DETECTION_2026-06-30.md |
| S2_DETECTION_LOG | ✅ ON | `1` (.env) | "" (empty → OFF) | Verbose S2 detection logging (observability). | Anchor-trial diagnostics (2026-06-12). | `backend/v9/systems/five_min/five_min_system.py:783`<br>`backend/v9/systems/five_min/five_min_system.py:898` |  |
| S2_REQUIRE_COT_AMT | 🔴 OFF | unset → "" (empty → OFF) | "" (empty → OFF) | Re-require footprint COT/AMT order-flow confirmation for S2 fires (S2 depends on S3). | DISABLED 2026-06-08 (S2 ⟂ S3): S3 is muted/broken (S3_MUTE / I-11); requiring it would block all S2. | `backend/v9/systems/five_min/five_min_system.py:642`<br>`backend/v9/systems/five_min/five_min_system.py:821` | STANDING-OFF — Michael sign-off to enable; CLAUDE.md §S2 ⟂ S3 |
...
| DAYTYPE_POSITION_GATE | ✅ ON | `1` (.env) | (no default) | Per day-type direction x price-vs-POC/IB gate; supersedes the legacy CCI/reactive gates. | ON. Correct on direction/location but PATTERN-BLIND — ignores its pattern arg (can't tell CONT from REV). | `backend/env_loader.py:79`<br>`backend/v9/systems/daytype_playbook.py:105`<br>`backend/v9/systems/daytype_position_gate.py:72`<br>(+1) | CASCADE_AUDIT §5 R2 |
...
| DEMO_EXECUTION_ENABLED | ✅ ON | `1` (.env) | "0" | Enable DEMO execution branch in the gateway. When OFF (default), no DEMO trades are placed. When ON, approved setups go to Sierra SIM via trade_command.json. | OFF (default). Pipeline 5 runtime gate. Michael sign-off required to enable. DEMO + Sim account ONLY. | `backend/main.py:751`<br>`backend/v9/gateway/trading_gateway.py:587` | docs/handoff/CC_PIPELINE5_2026-06-25.md |
...
| T1_LADDER_V2 | 🔴 OFF | unset → False (flag() default) | False (flag() default) | Swap T1 computation to a wider v2 ladder (t1_ladder_continuation_v2 / t1_reversal_multiplier_v2 / flag_relative_t1_v2). | REJECTED — backtest flat -$144 (struct -$624). Do NOT enable. | `backend/v9/systems/woodies/woodies_system.py:747`<br>`backend/v9/systems/stop_anchors/sizing.py:88`<br>`backend/v9/systems/five_min/five_min_system.py:1427` | REJECTED by backtest — do not enable; memory: project_trend_gate_t1_widen |
