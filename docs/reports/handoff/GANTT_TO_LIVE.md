...
| **1. Replay smoke run** | Re-run Prompt 28 after Phase 0 is fully green; prove the replay clock + all 6 systems still pass against the cleaned data path | P28 (re-run) | Phase 0 + Michael gate | **DONE — gate approved 2026-05-18** | Smoke checks PASS; full `tests/v9/` PASS (`1244 passed, 1 skipped`); Michael approved Phase 1 → Phase 2 |
| **2. Replay scenario pack** | Inject 10 offline scenarios (trend / balance / opening drive / S2 / S3 / S4 / killzone change / TPO context / degraded / pre_fire block) and verify expected reason trees and route/block outcomes | P29 | Phase 1 | **GREEN — waiting Phase 2 → Phase 3 gate** | 10/10 offline scenarios pass; `docs/reports/PROMPT29_REPLAY_SCENARIO_PACK.md` exists; full `tests/v9/` PASS (`1255 passed, 1 skipped`) |
