...
| 18 | **פאנל-תבניות FULL FIX (פקודת-מייקל 07-17 בוקר)**: לכל 19 התבניות — ההסבר + **מבנה-גאומטרי** + **מספר-נרות** חולצו מקוד-הדטקטורים האמיתי (ציטוט file:line פר-תבנית); 6 באגים תוקנו: החלטות לא-מודעות-כיוון, vetoed לא-ממופה, react-keys, פאנל-נעלם-ריק, שורות-S2 מתות, **4 שגיאות-דוקטרינה בטקסט** (Vegas divergence→cup-and-handle, INITIATIVE טווח-קבוע→דינמי, REACTIVE דרישת-COT/AMT שגויה, HFE ללא תווית-מושבת); tsc 0-חדשות | planHelp.ts · AllPatternsPlan.tsx · PatternsTab.tsx · FiveMinPlan.tsx | 🟢 נדחף | d79f1a48 |
| 19 | **N8 — CONFLUENCE_RI_ZLR נבנתה (flag-OFF, shadow-only)**: דטקטור-צירוף (S2 RI × S4 ZLR, אותו בר+כיוון, Δ≤1pt) → סטאפ-אחד 2-חוזים (פטור-FIXED_4 בנקודת-חנק יחידה), C1/C2=±4/±8, סטופ-מבני≤7pt, G-fresh על הסדרה הקנונית; ניתוב-shadow-כפוי גם ב-mode=live; CONFLUENCE_RI_ZLR_LIVE=stub-מסרב; playbook+stop_anchors+FLAG_REGISTRY+אינדקסים; **16 טסטים חדשים + 39/39 ירוק על פוסטגרס-אמת** | backend/v9/systems/confluence/ + trading_gateway + sierra_command | 🟢 נבנה — ממתין-push + פסיקת-מייקל להדלקת-SHADOW | דוח-סוכן + tests/v9/regression/test_confluence_ri_zlr.py |
...
| **N3** | **ZONE_LIMIT_ENTRY_V1** + סים (כניסה-מאוחרת-מדי) | מפרט+קוד | 🟡 חצי-חוסם בנוי flag-OFF 07-17 (שער no-late-entry ב-gateway: דריפט-ADVERSE>2pt / גיל>180s, fail-open, 15 טסטים ✅; חצי-ה-resting-limit+סים נשאר ל-cc-imac per הספק) |
