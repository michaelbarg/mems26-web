...
| **נקודת ציון**            | **P31 לפני P-S0** — בוקר 23.5: 🔒 **D-089 LOCKED** (S3=Firing, supersedes D-082+D-086) · types.ts + wrappers.py + Registry sync'd. ערב 22.5: Issue B ✅ · P31-02 S2 ✅ · T2 Woodies CCI ✅ · RCA-1 S6 direction-aware ✅ · `/trades` insight+Exit (OPEN state fix בקוד — דורש restart backend). **§15** Chart/CVD — UAT חלקי.                                                                                                                                                                                                                                                                                                                                                                                                                        |
| **הצעד הבא (בוקר)**       | (1) **העלה stack** — backend `:8000` + frontend `:3000` + bridge (לא רץ כרגע בבדיקה) · (2) **P31-01** P&L UAT ב-`/trades` ב-RTH · (3) **P31-PAT** תבנית אחת מול Sierra · (4) gate **P-S0** (D-089 ✅ pre-req cleared). מסמך מצב מערכות חי: `docs/reports/MEMS26_SYSTEMS_DECISIONS_REGISTRY_2026-05-23.md` · D-089 doc: `docs/decisions/D-089_S3_FIRING_LOCKED.md`                                                                                                                                                                                                                                                                                                                                                           |
| *(נקודת ציון קודמת)*      | **§15 — Chart RTH UAT live עם Michael ב-`localhost:3000`** — תיקנו 3 buggy behaviors בקווי POC הלבנים: (1) **infinite** → bounded ל-RTH window של היום (09:30→16:00 ET) · (2) **TZ drift 3h** מ-Local TZ → Intl.DateTimeFormat עם probe ל-ET · (3) **fixed end 16:00** → `**now+5min`** (עוקב המחיר) · (4) **HMR-safe global store** למחיקת קווי-orphans · (5) duplicate axis labels הוסרו · (6) **CVD history backfill** — `?history=1&limit=600` קורא מ-`v9_bars_cumulative_delta` (122 נקודות / 10h) במקום rolling 14 של ה-JSON. **27/27 tests PASS.** ממתין hard-refresh + backend restart לCVD. |
...
- קוד `STOP_HIT` + pytest PASS
- `POST /api/v9/trades/{id}/exit` → `TradeManager.close_trade` + pytest `test_trades_exit.py` PASS
- Backend רץ
- **restart backend** — PID **45069** (2026-05-21 ~14:00 ET)
- עסקת SHADOW אחת סגורה — **trade 697** (LONG · entry 7429.25 · T1+T2 · exit `manual` · DB `pnl_usd=56.25`)
- `curl /trades/log?types=shadow&limit=50` ↔ DB (trade 697 `pnl_usd=56.25` תואם · 21 rows · 0.02s) — Cursor 2026-05-21 צהריים
...
- `limit=50` ב-`journal/page.tsx`
- SQL: סינון `mode` לפני `LIMIT`; `load_only` בלי `cross_context` (~25KB/שורה)
- אינדקס `ix_v9_trades_mode_entry_ts` — migration `016_v9_trades_journal_index.sql`
- pytest `test_journal_compat_sql.py` PASS (3) + `test_journal_compat_routes.py` (2) + `test_journal_excursion.py` (1)
- שירות חדש `trade_excursion.py` — `price_high/low`, `mfe_pts/mae_pts`, `t1_closest_pts`, חלון 5m bars; query אחת לכל הרשימה (`prefetch_bars_for_trades`)
- שירות חדש `trade_legs.py` — `ENTRY/T1/T2/T3/STOP/EXIT` ממויינים לפי ts
- שירות מורחב `trade_context.py` — `compute_trade_pnl` עם `pnl_mode=closed/partial/open` + `contracts_pnl`
- עמודות חדשות ב-`/journal` UI — `Hi/Lo` בטבלה, `MAE/MFE` עם `—` כשאין נתונים, מודל פירוט עם Range Hi/Lo + בלוק רגליים
- `time curl …/trades/log?limit=50` post-restart — **0.02s** · 21 rows · trade 697 כולל range+legs (Cursor 2026-05-21 צהריים)
...
- תיקון `bars.py` — BarRouter על UPDATE (לא רק INSERT)
- regression: `test_post_5min_routes_bar_on_update`
- P31-02b in-process footprint (לא HTTP self-calls) + wire-up `main.py`
- P31-02c thread-leak + Woodies JSON datetime fix
- §6.5 live: dispatch 62–231ms (לא 8s)
- S2 audit Fix 1+2: COT/AMT מ-Sierra CDV · OVERNIGHT_MODE gate
- CC-C + RTH: `five_min/current` running/hydrated · `BarRouter: dispatch … for 5min` 12–28ms בלוג
- UAT RTH: S2 path + ירי (Michael 2026-05-22 ערב)
