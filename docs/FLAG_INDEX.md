...
_Generated 2026-06-30 15:22 · `.env` last modified 2026-06-29 16:32 · scan dirs: backend, bridge_
...
| S2_CVD_CONFIRM_V1 | 🔴 OFF | unset → "" (empty → OFF) | "" (empty → OFF) | CVD confirmation in S2 detection geometry: REACTIVE requires absorption (divergence from exhaustion), INITIATIVE requires delta alignment with breakout. | OFF (default, SHADOW only). 2026-06-29: INITIATIVE_SHORT fired without CVD confirmation despite +3,403 absorption. Michael sign-off required. | `backend/v9/systems/five_min/five_min_system.py:707`<br>`backend/v9/systems/five_min/five_min_system.py:740`<br>`backend/v9/systems/five_min/five_min_system.py:837`<br>(+1) | deferred; docs/handoff/CC_STAGE0_OPENING_FIRE_CVD_2026-06-29.md §Stage-2 |
| S2_DETECTION_LOG | ✅ ON | `1` (.env) | "" (empty → OFF) | Verbose S2 detection logging (observability). | Anchor-trial diagnostics (2026-06-12). | `backend/v9/systems/five_min/five_min_system.py:754`<br>`backend/v9/systems/five_min/five_min_system.py:869` |  |
| S2_REQUIRE_COT_AMT | 🔴 OFF | unset → "" (empty → OFF) | "" (empty → OFF) | Re-require footprint COT/AMT order-flow confirmation for S2 fires (S2 depends on S3). | DISABLED 2026-06-08 (S2 ⟂ S3): S3 is muted/broken (S3_MUTE / I-11); requiring it would block all S2. | `backend/v9/systems/five_min/five_min_system.py:634`<br>`backend/v9/systems/five_min/five_min_system.py:792` | STANDING-OFF — Michael sign-off to enable; CLAUDE.md §S2 ⟂ S3 |
...
| T1_LADDER_V2 | 🔴 OFF | unset → False (flag() default) | False (flag() default) | Swap T1 computation to a wider v2 ladder (t1_ladder_continuation_v2 / t1_reversal_multiplier_v2 / flag_relative_t1_v2). | REJECTED — backtest flat -$144 (struct -$624). Do NOT enable. | `backend/v9/systems/woodies/woodies_system.py:747`<br>`backend/v9/systems/stop_anchors/sizing.py:88`<br>`backend/v9/systems/five_min/five_min_system.py:1398` | REJECTED by backtest — do not enable; memory: project_trend_gate_t1_widen |
