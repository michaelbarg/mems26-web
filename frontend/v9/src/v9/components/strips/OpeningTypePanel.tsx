...
interface OpeningPanelData {
  ...
  live: { day_type: string | null; status: string | null; direction: string | null };
            ...
            <>
              ...
              {patterns.map((p) => {
                ...
                return (
                  <Pill
                    key={p.pattern}
                    text={`${p.pattern}${p.require_with_trend ? '·wt' : ''}`}
                    bg={st.bg}
                    fg={st.fg}
                    title={`${p.verdict}${p.group ? ` · ${p.group}` : ''}${p.require_with_trend ? ' · רק עם המגמה' : ''}`}
                  />
                );
              })}
            </>
