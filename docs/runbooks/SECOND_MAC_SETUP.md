...
- [ ] **חשבון-Teton (הכרעה ראשונה!):** האם ה-iMac סוחר חשבון **נפרד** (→ שתי המכונות
      סוחרות במקביל, אין כיבוי) או את **אותו** חשבון (→ מכונה אחת בלבד, המקבוק יורד).
      אמת: `pgrep -fl trade_activity_feed` — ה-`--account` חייב להתאים לחשבון-הסיירה
      של ה-iMac, לא של המקבוק. (07-12: ה-iMac יצא 37138283 = של המקבוק — לתקן אם שני-חשבונות.)
- [ ] **העברת DB (החלטה — ברירת-מחדל: לדחות):** לא-חוסם למחר — halt מתאפס יומית,
      לדג'ר קדימה-בלבד, tp_audit תקופתי. הרשומות בגיבוי המוצפן. אם רוצים המשכיות
      ל-tp_audit: במקבוק `pg_dump -Fc mems26 -f cut.dump` → ב-iMac `pg_restore -d mems26 cut.dump`.
- [ ] דריל-קצה-לקצה על סים ביום-שוק שקט + אישור-GO כתוב של מייקל
**העברת-Teton (חד-פעמית, לפני-פתיחה, רק אם חשבון-משותף):** לא מספיק לכבות שירותים —
במכונה-היורדת **סוגרים את סיירה / מנתקים מ-Teton** (שתי סיירות על חשבון אחד מתנתקות
הדדית — תקרית-הפיד של 07-11). רצף: (1) במקבוק — סגור סיירה + `MEMS26_CONTROL stop`;
(2) ב-iMac — חבר סיירה ל-Teton (סיסמה) + ודא feed חי; (3) fire_drill 🟢 עם feed.
אחרי ה-GO המקבוק נשאר פיתוח בלבד.
