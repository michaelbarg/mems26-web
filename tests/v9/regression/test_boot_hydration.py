...
def test_hydrate_live_pnl_restores_counters():
    """After hydration, gateway counters match DB state."""
    gw = _make_gateway()
    ...
    with patch("backend.v9.db.read.read_scalar", side_effect=[-25.0, 3]), \
...
def test_hydrate_live_pnl_no_trades():
    """Hydration with no trades keeps counters at 0."""
    gw = _make_gateway()
    with patch("backend.v9.db.read.read_scalar", side_effect=[0.0, 0]), \
...
def test_hydrate_live_pnl_db_error_is_nonfatal():
    ...
    gw = _make_gateway()
    with patch("backend.v9.db.read.read_scalar", side_effect=Exception("DB down")):
...
def test_hydrate_all_wins_zero_consecutive_losses():
    ...
    with patch("backend.v9.db.read.read_scalar", side_effect=[150.0, 2]), \
         patch("backend.v9.db.read.read_all", return_value=mock_rows):
        gw.hydrate_live_pnl()
