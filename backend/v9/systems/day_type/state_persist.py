...
def persist_state_row(
    ...
    try:
        ...
        # §9ב invariant: at IB lock (first B2 persist only), rib must be ~1.0.
        # rib < 0.99 at lock means the classifier read stale bar snapshots.
        _stage_val = state.stage.value if hasattr(state.stage, "value") else str(state.stage)
        if (_stage_val == "B2"
                and not getattr(app_state, "_rib_invariant_fired", False)
                and last_cls_result is not None):
            ...
            if _rib_check is not None and float(_rib_check) < 0.99:
                logger.error(
                    "[S1] rib=%.3f at IB lock — input is not a closed bar",
                    float(_rib_check))
                app_state._daytype_input_suspect = True
            app_state._rib_invariant_fired = True
