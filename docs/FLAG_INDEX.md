...
_Generated 2026-06-29 19:43 · `.env` last modified 2026-06-29 16:32 · scan dirs: backend, bridge_

**Legend:** ✅ ON · 🔴 OFF · 🟡 ON·inert (set ON but superseded at runtime) · 🔢 numeric param · ⚪ not built.

**Summary:** 67 documented · 43 ON (of which 3 inert) · 10 OFF (3 standing-OFF) · 13 numeric params · 1 awaiting backtest · 1 rejected · 1 not built.
...
| Flag | State | Current | Code default | What it does | Why / state | Where (file:line) | Notes |
|------|-------|---------|--------------|--------------|-------------|-------------------|-------|
| DAYTYPE_PATTERN_AWARE_V1 | 🔴 OFF | unset → "0" | "0" | Pattern-family gate: balanced day → REV only (block CONT); trend/variation → CONT only (block REV). Post-IB-lock. | OFF (default, SHADOW only). 2026-06-29: INITIATIVE_SHORT fired on Normal day (should be REV-only). Michael sign-off required. | `backend/v9/systems/daytype_position_gate.py:101` | deferred; docs/handoff/CC_STAGE1_PATTERN_AWARE_GATE_2026-06-29.md |
