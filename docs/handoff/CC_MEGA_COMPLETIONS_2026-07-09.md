...
## עדכון 22:3x — D3 התקבל ✓ · המשימה הבאה: הצעת-יעדים לפסיקה
‏TP_AUDIT_2026-07-09.md אומת (276 עסקאות/30 יום). המשך: תרגם את הטבלה ל**הצעת
targets פר-תבנית×סוג-יום** לפסיקת מייקל מחר לפני הפתיחה: לכל תא עם n≥5 — ‏T1 מוצע
(עוגן: ‏p25-p50 של ה-MFE, לעולם לא מעבר ל-IB ביום לא-מסויג per TP-1), ‏delta מול היום,
והשפעה צפויה על hit%. פורמט: טבלה אחת + 5 שורות "השינויים הגדולים" (TLB/ZLR קרובים מדי ·
FAMIR/BULL_FLAG רחוקים מדי · REACTIVE_LONG-Variation חריג). בלי לגעת ב-targets.yaml עד פסיקה.
אחר-כך: CERT test-debt (עדכון 22:2x) → D1 (DLL) → D4 → D5 → writer-revive.

## עדכון 22:4x — FIX-8 (נכנס אחרי הצעת-היעדים וחוב-הטסט)
**FIX-8 — ATR-בוסר בשעה הראשונה.** ראיה 16:40: ‏StopResolver ‏rung1 ‏dist=10.75 (סטופ-דרייב
מבני נכון) נדחה כי ‏cap=8.22 — ‏ATR14 מחושב על 1-2 ברי-סשן זעירים → תקרות לא-ריאליות →
‏no_stop_in_band → סטופ-גלם 26 נק' → ‏R:R קבור. תקן: עד שקיימים 14 ברי-סשן, ‏ATR14 =
‏max(חלקי-סשן, ‏ATR מ-N ברי-הסגירה של אתמול מאותה טבלה) — משפיע על תקרת-הסטופ (atr_stop),
‏cap_risk_points (sizing) וסובלנות-האישור (ENTRY_CONFIRM_TOL). דגל EARLY_ATR_FLOOR_V1
‏(default OFF → פסיקת מייקל), טסט אנטי-טאוט עם פיקסטורת 16:40 האמיתית. מלא:
docs/reports/MISSED_TRADES_2026-07-09.md.
