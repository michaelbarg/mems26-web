...
| G-19 | DLL | PLACE_STOP (Exit-family) r=-1 — `sc.SellExit/BuyExit` תומך MARKET בלבד, לא STOP → ORPHAN לא-מוגן | 🟢 **CONFIRMED (cc-sim A1.6)** | `MES_AI_DataExport.cpp:1389-1423`. אותה מחלקה כמו op=EXIT השבור. **תיקון: `sc.SubmitOrder` standalone STOP + שומר reduce-only** (CC_MEGA_PROMPT_2026-07-19.md משימה-1) → RB+re-sim | מייקל→cc | 07-19 |
