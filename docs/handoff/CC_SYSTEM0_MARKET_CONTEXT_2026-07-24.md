...
## Phase A3 — איחוד-שערים: MarketContext = סמכות-הכיוון היחידה (פסיקת-מייקל 07-29 "תיקון לטווח ארוך")
**הטריגר:** מבוי-סתום 07-29 21:20-21:25 — playbook (day_dir=UP) חסם שורטים בעוד cont_trend_filter (sustained DOWN)
חסם לונגים באותו רגע → אפס-ירי לשני הכיוונים ביום-V. שורש: לכל שער מדד-כיוון פרטי עם lookback שונה.
**העיקרון: מוח אחד, הרבה ידיים.** אף שער לא מחשב כיוון בעצמו:
1. **מפת-שערים (audit קודם!):** cont_trend_filter · DIRECTION_LSMA_VETO/lsma_flat · playbook-day_dir · RESPONSIVE/NEVERFADE ·
   release-gate(trend_bypass) · opening-dir-fusion — לכל אחד: KEEP-כצרכן / RETIRE-ככפול. טבלה בדוח.
2. **כל KEEP עובר לקרוא `get_market_context().day_bias`** (מקור-יחיד); המדדים-המקומיים נמחקים (לא "נשמרים ליתר-ביטחון").
3. **הרחבת-החוזה: שדה `phase`** ∈ trend/rotation/**reversal_V**/rebalancing — ביום-V המעבר DOWN→REVERSAL_UP הוא החלטה
   אחת מפורשת (escalation-lock, חצייה-מאושרת-נפחית של המהלך), במקום שני שערים חלוקים.
4. **מטריקת-אנטי-רגרסיה:** ל-eod_review נוסף מונה "חסימות-כיוון-סותרות" (שורט-נחסם-כי-UP + לונג-נחסם-כי-DOWN באותם
   30 דק') — חייב 0; >0 = 🔴 בדוח-היומי. זה גלאי-המחלקה של 60/0.
5. **rollout בטוח:** דגל `MARKET_CONTEXT_AUTHORITY_V1` (OFF) → מצב-shadow שמתעד פר-שער "מה-הייתי-מחליט-תחת-המקור-האחיד"
   מול בפועל למשך 2-3 סשנים → הצגת-דלתא למייקל → הדלקה בפסיקה אחת. **אין-מחיקת-מדד-מקומי לפני שה-shadow מוכיח שוויון/שיפור.**
