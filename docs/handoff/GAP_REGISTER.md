...
| G-19 | DLL | PLACE_STOP (Exit-family) r=-1 — `sc.SellExit/BuyExit` תומך MARKET בלבד, לא STOP → ORPHAN לא-מוגן | 🟢 **CONFIRMED (cc-sim A1.6)** | `MES_AI_DataExport.cpp:1389-1423`. אותה מחלקה כמו op=EXIT השבור. **עדכון:** 3 גישות נכשלו — ACSIL אין סטופ-רסטינג עצמאי (Exit=MARKET · Entry+STOP=r=-1 · SubmitOrder לא-קיים). send=0 נכון-בסים (`:174`). **תיקון-מאושר: הגנת-יתום = ניטור-backend→יציאת-שוק (`FLATTEN_ORPHAN`, Exit+MARKET מוכח `:1487`)**, לא סטופ-רסטינג | מייקל→cc | 07-19 |
